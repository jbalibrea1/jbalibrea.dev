...
- [Qué es Proxmox](#qué-es-proxmox)
- [Qué es Docker](#qué-es-docker)
- [Qué es Portainer](#qué-es-portainer)
...
Proxmox es una plataforma de virtualización de código abierto que permite crear y gestionar máquinas virtuales y contenedores. Se entra a traves de la dirección IP del servidor y se gestiona desde un navegador web, no vas a necesitar un monitor conectado al "servidor" o un teclado.
...
En mi caso ejecuto todo a traves de portainer (no es algo imprescindible), en caso de no usar portainer también puedes levantar los contenedores con `docker compose up -d` en la carpeta donde tenemos el docker-compose.yml, se creará un contenedor y tendríamos nuestro nginx proxy manger corriendo.

#### Algunos de los servicios que tengo corriendo son:
...
Para instalar Portainer, se instala a traves de docker con el siguiente comando:
